...
int main(int argc, char* argv[]) {
    try {
        ...
        std::cout << "try to get a camera!" << std::endl;
        ...
        //do {
            ...
            std::cout << "cam img  captured!" << std::endl;
            double t1 = static_cast<double>(cv::getTickCount());
            std::vector<HumanPose> poses = estimator.estimate(image);
            double t2 = static_cast<double>(cv::getTickCount());

            std::cout << "pose points estimated on device!" << std::endl;
            ...
            cv::imwrite("ICV_Human_Pose_Estimation.jpg", image);
